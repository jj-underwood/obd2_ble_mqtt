...
class OBD2Data {
 public:
  enum ValueType { STRING, FLOAT, BOOL };
  
  explicit OBD2Data(const std::string &value) : type_(STRING), string_value_(value) {}
  explicit OBD2Data(const char *value) : type_(STRING), string_value_(value) {}
  explicit OBD2Data(float value) : type_(FLOAT), float_value_(value) {}
  explicit OBD2Data(bool value) : type_(BOOL), bool_value_(value) {}

  ValueType get_type() const { return type_; }
  const std::string &get_string() const { return string_value_; }
  float get_float() const { return float_value_; }
...
class OBD2BLEClient : public esphome::ble_client::BLEClientNode, public Component, public esphome::mqtt::CustomMQTTDevice {
 ...
 public:
  ...
  // Sensor management
  void add_task_for_sensor(sensor::Sensor *sensor, const std::string &can_id, const std::string &mode, const std::string &pid, const std::string &bits);
  ...
  void add_task_for_binary_sensor(binary_sensor::BinarySensor *sensor, const std::string &can_id, const std::string &mode, const std::string &pid, const std::string &bits, const std::string &code);
 ...
 protected:
  ...
  std::string get_o2_sensor_presence(const uint8_t &num);
  std::string get_standard_name(const uint8_t &num);
  std::string get_fuel_type(const uint8_t &num);
  std::string get_auxiliary_io_supported(const std::vector<uint8_t> &data);
  float get_ect(const std::string &bits, const std::vector<uint8_t> &data);
  float get_fuel_pressure_control_system(const std::string &bits, const std::vector<uint8_t> &data);
  std::string get_vin(const std::vector<uint8_t> &data);
  std::string get_cid(const std::vector<uint8_t> &data);
  std::string get_cvn(const std::vector<uint8_t> &data);
  float get_ipt(const std::string &bits, const std::vector<uint8_t> &data);
  std::string get_ect_name(const std::vector<uint8_t> &data);
  bool is_bit_set(const uint8_t &value, const int &position);
  std::string get_ipt_name(const size_t &num);
  void add_string(std::string &base_str, const std::string &add_str, const std::string &delimiter);
  ...
  // MQTT handlers
  void publish_message(const std::string &topic, const std::map<std::string, OBD2Data> &data);
